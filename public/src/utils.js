...
(function (factory) {
	...
	var utils = {
		...
		date: function (date) {
			...
			const formattedDate = `${month}-${_date} ${hours}:${minutes}:${seconds}.${milliseconds}`;
		...
		animateNumbers: ($target, min = 0, max = 0, symbol) => {
			$({ value: min }).animate(
			  {
				value: max,
			  },
			  {
				...
				step: function () {
				  ...
				  $target.text(symbol + roundedValue);
				},
				complete: function () {
				  const finalRoundedValue = (this.value);
				  $target.text(symbol + finalRoundedValue);
				},
			  }
			);
		  }
	...
	(() => {
		let r = r => function (o = "APP", ...e) {
			if (!utils.log) return !1;
			r.call(console, `${utils.date()} [${o}]`, ...e)
		};
		console.log = r(console.log), console.warn = r(console.warn), console.error = r(console.error), console.info = r(console.info), console.debug = r(console.debug), console.trace = r(console.trace)
	})();

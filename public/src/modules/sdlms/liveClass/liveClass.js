...
define("sdlms/liveClass/liveClass", [
	'api', "sdlms/eaglebuilder", "sdlms/threadbuilder", "sdlms/feedbacks", "sdlms/spreadsheet", "sdlms/sharer"
], function (api, eb, threadbuilder, feedbacks, spreadsheet) {
	var LIVE_CLASS = {}
	window.students = [];
	console.log("liveClass.js loaded");
	let session = ajaxify.data.Sessions[0];
	...
	LIVE_CLASS.init = (data) => {
		...
		let checkLoading = setInterval(() => {
			if (LIVE_CLASS.ebLoaded && LIVE_CLASS.memberLoaded) {
				app.loader(0);
	...
	LIVE_CLASS._template = (part, data = {}) => {
		let components = {
			student: () => {
                ...
                <div class="col-8 d-flex pl-0" user-info>
                    ...
                    <div class="user-info pl-2 d-flex flex-column">
                        ...
                        <div class="user-last-activity hidden-on-collapsed sdlms-sub-text-primary-12px font-weight-500" auto-update-time-ago="${moment(data.stats ? data.stats.eb.timestamp : 0)}">${moment(data.stats ? data.stats.eb.timestamp : 0).fromNow()}</div>
                    </div>
                </div>
			...
			studentSearch: () => {
				return ` <div class="sdlms-assets-selection-user-list" data-students-search data-fullname="${data.fullname}" data-displayname="${data.displayname}" data-username="${data.username}" data-uid="${data.uid}" data-is-teacher="${data.isTeacher}" data-role="${data.role}" >
	...
	LIVE_CLASS.track = () => {
		...
		function updateState(elem) {
			...
			$container.find('.tracker-state').each(function () {
				let $label = $(this).find('.tracker-state-label');
				let data = $label.data();
		...
		if (ajaxify.data.isTeacher) {
			...
			$container.find('[sdlms-session]').off('click').on('click', function () {
				if ($(this).attr('sdlms-session') != 'stop' || confirm("Stop Current Session?")) {
					let state = $(this).attr('sdlms-session');
					updateState(state);
	...
	LIVE_CLASS.getAsset = async (uid, type) => {
		if (type == 'tb') {
			...
			let userSP = await api.get(`/sdlms/spreadsheet?tid=${tid}&uid=${uid}`, {});
			let pid = userSP.pid;
	...
	LIVE_CLASS.initSpreadSheet = async () => {
		...
		let userSP = await api.get(`/sdlms/spreadsheet?tid=${tid}`, {});
		let pid = userSP.pid;
		userSP.data = userSP.data || {};
		let readonly = (userSP.data || {}).readonly;
		if (!ajaxify.data.isTeacher && ajaxify.data.isStudent) {
	...
	LIVE_CLASS.events = () => {
		...
		socket.on('meta.live.feedback', (data) => {
			if (data.asset_owner_uid == uid && data.tid == tid) {
				let d = $('[data-students-search][data-uid="' + data.creator + '"]').data();

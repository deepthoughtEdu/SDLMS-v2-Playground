 ...
 */
 ...
 class threadBuilder {
	...
	restore() {
		...
		if (null != localStorage.getItem(`tb_draft_${this.tid}_${this.data.uid}`)) {
			...
			let data = JSON.parse(localStorage.getItem(`tb_draft_${this.tid}_${this.data.uid}`));
	...
	builder(target = "body") {
		...
		socket.on('meta.live.joined', (data) => {
			if (data.tid == $that.tid) {
				let i = $that.students.findIndex(x => x.uid == data.uid);
				if (i == -1) {
	...
	thread() {
		...
		let components = {
				...
				<div class="col-6 pl-0">
					...
					<div class="d-flex sdlms-subthread-actions pt-3 justify-content-end">
						<div class="sdlms-custom-radio-image" data-title="Eureka Moment" title-bottom>
							<input type="checkbox" ${!!!Number(data.eureka) || 'checked'}  name="thought-eureka" id="${temp_id}1">
							<label for="${temp_id}1">
						...
						<div class="sdlms-custom-radio-image"data-title="Answers" title-bottom>
							<input type="checkbox" ${!!!Number(data.answer) || 'checked'}  name="thought-answer" id="${temp_id}2">
							<label for="${temp_id}2">
						...
						<div class="sdlms-custom-radio-image"data-title="Questions" title-bottom>
							<input type="checkbox"  ${!!!Number(data.question) || 'checked'}  name="thought-question" id="${temp_id}3">
							<label for="${temp_id}3">
						...
						<div class="sdlms-custom-radio-image" data-title="Root of thought" title-bottom>
							<input type="checkbox"   ${!!!Number(data.root) || 'checked'} name="thought-root" id="${temp_id}4">
							<label for="${temp_id}4">
			...
			save: () => {
				return `<div class="sdlms-threads-actions mt-4  col-md-11 mx-auto d-flex justify-content-end">
	...
	create(data = null) {
		...
		$target.on('change','[type="checkbox"][name="thought-question"]',function(){
			 ...
			 $(window).trigger('sdlms:thought.question',data)
		});
		...
		$target.on('click', '.add-thread-credit', function (e) {
			let student = $that.students.find(e => e.uid == $(this).data('uid'));
			let $ele = $(this).parents('[thread-credit]').find('[thread-credited-user]');
			$ele.empty();
	...
	getJSON() {
		...
		$target.find("[thread]").each(function () {
			...
			_thread.title = $(this).find('[thread-name="title"]').text();
			_thread.duration = $(this).find('[thread-name="duration"]').text();
			_thread.emotions = $(this).find('[name="emotions"]').val();
			_thread.summary = {};
			_thread.credit = $(this).find('[thread-credited-user]').data() || {};
			_thread.summary.content = $(this).find('[summary] [name="content"]').val();
			_thread.summary.title = $(this).find('[summary] [summary-title="name"]').text();
			$(this)
				.find("[subthread]")
				.each(function () {
					let subthread = {};
					subthread.id = `${_thread.id}-${$(this).index() + 1}`;
					subthread.title = $(this).find('[subthread-name="title"]').text();
					subthread.interpretation_title = $(this).find('[interpretation-name="title"]').text();
					subthread.content = $(this).find('[name="content"]').val();
					subthread.interpretation = $(this).find('[name="interpretation"]').val();
					subthread.category = $(this).find('[name="category"]').val();
					subthread.process = $(this).find('[name="process"]').val();

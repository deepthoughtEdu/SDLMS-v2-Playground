class eagleBuilder {
	...
	restore() {
		...
		if (null != localStorage.getItem(`eb_draft_${this.tid}_${this.data.uid}`)) {
			...
			let data = JSON.parse(localStorage.getItem(`eb_draft_${this.tid}_${this.data.uid}`));
	...
	create(data = null) {
		...
		if (data && data.tracks) {
			...
			let $container = $target.find('.sdlms-threads-container');
		...
		function reIndex() {
			$target.find('[thread]').each(function () {
				let $this = $(this);
				let index = $(this).parent().children('[thread]').index(this);
				...
				$this.find('[subthreadcontainer] [subthread]').each(function () {
					let index = $(this).parent().children('[subthread]').index(this);
					$(this).find('index').text(index + 1);
				});
				let subthreads = $this.find('[subthreadcontainer] [subthread]').length;
				if (subthreads > 1) {
			...
			let threads = $target.find('[thread]').length;
			if (threads > 1) {
		...
		$target.on("click", "[remove-thread]", function () {
			...
			$(this).parents("[thread]").next("[transitions]").first().remove();
		...
		$target.on("click", "[add-subthread]", function () {
			...
			$target.removeClass('sdlms-form-validated');
			let index = $(this).parents("[thread]").find('[subthreadcontainer]').find('[subthread]').length;
		...
		$newThread.off("click").on("click", function () {
			...
			$target.removeClass('sdlms-form-validated');
			let index = $container.find("[thread]").length;
	...
	getJSON() {
		...
		eaglebuilder.meta.introduction = $target.find('[meta] textarea[name="content"]').val();
		eaglebuilder.meta.title = $target.find('[meta] [name="title"]').text();
		eaglebuilder.meta.duration = $target.find('[meta] [name="duration"]').text() || 0;
		...
		eaglebuilder.conclusion.title = $target.find('[conclusion] [name="title"]').text();
		eaglebuilder.conclusion.content = $target.find('[conclusion] [name="content"]').val();
		eaglebuilder.conclusion.duration = $target.find('[conclusion] [name="duration"]').text() || 0;
		...
		$target.find("[thread]").each(function () {
			...
			thread.type = "track";
			thread.duration = $(this).find('[thread-name="duration"]').text() || 0;
			thread.duration = isNaN(Number(thread.duration)) ? 0 : Number(thread.duration);
			totalTime += thread.duration;
			thread.arguement = $(this).find("[arguement] textarea[name='content']").val();
			...
			$(this)
				...
				.each(function () {
					...
					subthread.title = $.trim($(this).find('[subthread-name="title"]').text());
					subthread.duration = 0;
					subthread.content = $.trim($(this).find('[name="content"]').val());
			...
			thread.transitions.title = $.trim($transition.find('[transition-name="title"]').text());
			thread.transitions.content = $.trim($transition.find('[name="content"]').val());
			thread.transitions.duration = $transition.find('[transition-name="duration"]').text() || 0;
	...
	reader(data) {
		...
		$target.find('[show-more]').off('click').on('click', function () {
			...
			if ($.trim($(this).text()).toLowerCase() == "see more")
				$(this).text("See less")
			else
				$(this).text("See more");
		});
		...
		if (this.data.tracking) {
			...
			$target.find('[convert-to-minute]').each(function () {
				let $this = $(this);
				let time = isNaN($.trim($this.text())) ? 0 : $.trim($this.text());
			...
			$target.find('[track-mode="online"]').each(function () {
				...
				if ($(this).data('threadProgress') > 0) {
					let percentage = $(this).data('threadProgress');
					let threadDuration = $(this).data('threadDuration');
					let value = ((threadDuration * percentage) / 100);
					$(this).parents('.sdlms-thread').find('[data-name="percentageToTime"]').css('color', (percentage > 100 ? 'var(--primary-danger-color)' : '')).text(formatTime(value * 60) + ' /');
				}
				tracker.push($.extend({}, {
					$elem: $(this),
				}, $(this).data()));
			...
			if (tracker.length) {
				...
				let currentIndex = tracker.findIndex(e => e.threadStatus == -1);
				...
				let prevIndex = tracker.findLastIndex(e => e.threadStatus == 1);
				...
				let interval = setInterval(() => {
					...
					let value = ((current.threadDuration * percentage) / 100);
				...
				$target.find('[mark-as-completed]').off('click').on('click', function () {
					let $id = $(this).attr('mark-as-completed');
					let $bar = $(`[track-mode="online"][data-id="${$id}"]`);
					let $data = $bar.data();
					...
					require(['api'], function (api) {
						api['put'](`/sdlms/${$that.tid}/eaglebuilder/${$that.data.id}`, session).then((res) => {
							...
							let eb = new eagleBuilder(_payload);

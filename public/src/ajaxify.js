...
(function () {
	...
	ajaxify.go = function (url, callback, quiet) {o
		 ...
		 * @author Deepansu
		 * @date 2022-01-31
		 * @description Adding custom route change event so that we can listen whenever route has changed.. with old and new route
		 * 
		*/
		$(window).trigger('routeChange', { url: (url).startsWith('/') ? (url).replace('/', '') : (url), previousUrl: (app.previousUrl).startsWith('/') ? (app.previousUrl).replace('/', '') : (app.previousUrl) })
	...
	ajaxify.coldLoad = function () {
		...
		console.log('AJAXIFY', 'Cold load complete');
	};
		...
		require(['translator', 'benchpress'], function (translator, Benchpress) {
			Benchpress.render(tpl_url, data)
				...
				.then(function (translated) {
					...
					$('body').removeClass('has-left-sidebar has-right-sidebar')
	...
	ajaxify.end = function (url, tpl_url) {
		...
		if (ajaxify.data.menus) {
			ajaxify.data.menus.left && $('body').addClass('has-left-sidebar');
			ajaxify.data.menus.right && $('body').addClass('has-right-sidebar');
		}
			...
			var process = function () {
				if (!e.ctrlKey && !e.shiftKey && !e.metaKey && e.which === 1) {
					if (internalLink) {
						...
						// if in an iframe, open in new tab
						if (window.parent && window.parent !== window) {
							let confirmOpeningInParent = confirm(`You are about to open an external link. Do you want to continue?`);
							confirmOpeningInParent && window.open(this.href, '_blank');
							e.preventDefault();
						}

...
define("forum/sdlms/dtthon/DtThon", ["api"], function (api) {
	var DtThon = {};
	DtThon.init = () => {
		$("[tabheader]")
			.off("click")
		...
		DtThon.paginateMicroDashboard(`/apps/project?isRecruiter=true`, {
		...
		if (app.user.isAdmin) {
			$("[CreateNewproject]")
				...
				.on("click", function () {
					...
					location.href = location.origin + "/monitor/";
				});
		} else {
			$("[CreateNewproject]").parent().addClass("change-class");
			$("#thStudent1").text("Joining date");
			$("#thStudent2").text("Deadline");
			$("#thStudent3").text("Project status");
			$("#thStudent4").text("review Status");
		}
	...
	if (app.user.isAdmin) {
		$('body').on('click','.dropdown-item',function(){
			...
			if(txt == 'Draft'){
				...
				DtThon.paginateMicroDashboard(`/apps/project`, {
					parent: "sdlms-my-microdashboard-table-body",
				});
			}
		})
	...
	$(".Dashboard1-state-main")
		...
		.on("click", function () {
			...
			if (url) {
				DtThon.paginateMicroDashboard(url, {
					parent: "sdlms-my-microdashboard-table-body",
				});
				return;
			}
	...
	$(".Dashboard1-state-detailed")
		...
		.on("click", function () {
			...
			console.log(url);
			if (url) {
				DtThon.paginateMacroDashboard(url, {
					parent: "sdlms-my-macrodashboard-table-body",
				});
				return;
			}
	...
	DtThon.paginateMicroDashboard = (url, data = {}) => {
		console.log(url);
	...
	DtThon.paginateMacroDashboard = (url, data = {}) => {
		api.get(url, {}).then((res) => {
			console.log(res);
			...
			res.data.map((ev, index) => {
               ...
               <td class="sdlms-my-upcoming-session-table-related-sessions font-weight-500 sdlms-text-black-18px">
			   --</td>
               <td class="sdlms-my-upcoming-session-table-related-sessions font-weight-500 sdlms-text-black-18px" id="${ev.pid}_Storyboard">
                   <svg width="19" height="18" viewBox="0 0 19 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                       <path

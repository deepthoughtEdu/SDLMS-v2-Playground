...
module.exports = function (Groups) {
	 ...
	 * @param {string} uid - uid of parent 
	 * @details gets array of uids of childrens of a parent's uid
	 */
	Groups.getChildren = async function (uid) {
		const collectionName = db.collections.DEFAULT;
		//when parent registers we will have to set this object, and also we will have to add his children to it
		const parentData = await db.findField(collectionName, {_key: `parent:${uid}:parentData`});

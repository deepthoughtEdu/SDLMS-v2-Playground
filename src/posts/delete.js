...
module.exports = function (Posts) {
	...
	Posts.purge = async function (pid, uid) {
		...
		if (postData.typeOfAsset) 
			{ 
				if (!postData.typeOfAsset.includes('topic') && !postData.typeOfAsset.includes('post')){
					try { 
						await db.decrAssetcount(collectionName, `user:${postData.uid}`, postData.typeOfAsset); 
						await db.decrAssetTime(collectionName, `user:${postData.uid}`, postData.typeOfAsset, postData.timestamp - postData.startedAt); 

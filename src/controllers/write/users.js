...
Users.assignRoles = async (req, res) => {
	const key = `user:${req.params.uid}`;
	const role = req.params.role;
	await api.users.assignRole(key, role);
	helpers.formatApiResponse(200, res);
}
...
Users.getUser = async function (req, res) {
	let uid = req.params.uid;
	if(!uid) return helpers.formatApiResponse(400, res);
	let _user = await user.getUsers([uid]);
	_user = _user[0] || {};
	return helpers.formatApiResponse(200, res, _user);
}
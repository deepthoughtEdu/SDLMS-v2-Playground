...
apiController.loadConfig = async function (req) {
	...
	if (req.loggedIn) {
		([settings, isAdminOrGlobalMod, config.isAdmin] = await Promise.all([
...
function getBaseStyles (req) {
	let styles;
	let coreStyles = [{
		name: 'sdlms',
		href: nconf.get('relative_path') + '/assets/styles.css?' + meta.config['cache-buster'] || '',
		referrerpolicy: 'no-referrer',
		integrity: '',
		crossorigin: 'anonymous'
	}, {
		name: 'mobile',
		href: nconf.get('relative_path') + '/assets/mobile.css?' + meta.config['cache-buster'] || '',
		referrerpolicy: 'no-referrer',
		integrity: '',
		crossorigin: 'anonymous'
	}];

	let uriToArray = req.url.split('/') || [''];
	let isMobile = (uriToArray.length > 1) ? (uriToArray[1] == 'mobile') : false;
	
	if (isMobile) {
		styles = coreStyles.filter((style) => style.name != "sdlms");
	} else {
		styles = coreStyles.filter((style) => style.name != "mobile");
	}

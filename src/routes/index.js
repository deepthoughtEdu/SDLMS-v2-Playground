...
function categoryRoutes(app, middleware, controllers) {
	...
	setupPageRoute(
		app,
		"/category/:category_id/:slug/:topic_index",
		middleware,
		[],
...
module.exports = async function (app, middleware) {
	...
	router.all(
		...
		ensureLoggedIn.ensureLoggedIn(
			nconf.get("relative_path") + "/login?local=1"
		),
...
function addCoreRoutes(app, router, middleware) {
	...
	profileRoutes(router, middleware, controllers);
	userRoutes(router, middleware, controllers);
	groupRoutes(router, middleware, controllers);
	applicationManager(router, middleware, controllers);
	dtpen(router, middleware, controllers);
	...
	var statics = [
		...
		{
			route: "/plugins",
			path: path.join(__dirname, "../../build/public/plugins"),
		},
	];
	var staticOptions = {
	...
	statics.forEach(function (obj) {
		app.use(
			...
			middleware.trimUploadTimestamps,

...
SocketUser.reset.commit = async function (socket, data) {
	 ...
	 * @author imshawan
	 * @date 25-02-2022
	 * @description Incomming OTP from the client side is first checked in database, if matches with a token (code)
	 * then it is forwarded to the password reset controllers
	 */

	const collectionName = db.collections.DEFAULT;
	const resetPayload = await db.findField(collectionName, { otp: data.code.toString() })
	if (!resetPayload) {
		throw new Error('[[error:reset-code-not-valid]]');
	}
	const token = resetPayload.code;
	
	const [uid] = await Promise.all([
		db.getObjectField('reset:uid', token),
		user.reset.commit(token, data.password),

 ...
 */
			...
			await Promise.all(Items.map(async (item) => {
				if (item) {
					...
					if (record && record.uid) {
						record.user = await User.getUserFields([record.uid], ['username', 'fullname']);
					}
					records.push(record);
				}
			}));
 ...
 appApi.getCategory = async (data) => {
	...
	const categoryTypes = SDLMS.categoryTypes.map((elem) => elem.toLowerCase());

	const collectionName = db.collections.DEFAULT;
	const categoryType = data.query.type;
	const keys = { categoryType: { $exists: true } };

	if (categoryType) {
		if (!categoryTypes.includes(categoryType.toLowerCase())) {
			throw new Error("Invalid category-type: " + categoryType);
		}

		keys['$or'] = [{categoryType}, {categoryType: categoryType.toLowerCase()}];
	}

	const categoryFields = ["name", "cid", "categoryType", "parentCid"];
	// , $or: [{'parentCid' : {$exists:false}}, {'parentCid' : 0}]
	let category = await db.findFields(collectionName, keys, categoryFields);

	if (data.query.name) {
		category = await db.findFields(collectionName, { name: data.query.name }, categoryFields);
	}

	await Promise.all(category.map(async (elem) => {
			let subCategory = await db.findFields(collectionName,
				{$or: [{ parentCid: elem.cid }, { parentCid: String(elem.cid) }]},
				categoryFields
			);
	...
	category.forEach((elem) => {
		...
		else if (elem.parentCid && parseInt(elem.parentCid) < 1) {
			categoriesData.push(elem);
		}
	})
...
appApi.createRoom = async (req) => {
	...
	moderators = moderators.map(moderator => parseInt(moderator))
	...
	const Data = {
		...
		min_rigor_rank: Number(req.body.min_rigor_rank) || 0,
	}
...
appApi.getRoom = async (req) => {
	...
	if (roomId.length == 1) {
		...
		return await Messaging.getRoomsData([roomId])
	}

}

appApi.deleteAllRooms = async (req) => {
	const collectionName = db.collections.DEFAULT;
	const data = {
		type: "discuss_room",
	}
...
appApi.reportThread = async (req, isReported) => {
	const collectionName = db.collections.DEFAULT;
	const uid = parseInt(req.uid);
	const { roomId, messageId, reportedFor } = req.body;
	const key = { _key: `message:${messageId}` };

	let message = await db.findField(collectionName, key);
	if (!message) throw new Error("Invalid message Id");
	if (message.roomId != roomId) throw new Error("Invalid room Id");

	if (isReported) {
		if (!message.reportedBy) {
			await db.updateField(collectionName, key, { $set: { 'reportedBy': [{ uid, reportedFor }], isReported } }, { upsert: false });
		} else {
			await db.updateField(collectionName, key, { $addToSet: { 'reportedBy': { uid, reportedFor } } }, { upsert: false });
		}
	} else {
		await db.updateField(collectionName, key, { $set: { 'reportedBy': [], isReported } }, { upsert: false });
	}

	return { reported: isReported }
}

appApi.getReportedThreads = async (req) => {
	const collectionName = db.collections.DEFAULT;
	const uid = parseInt(req.uid)
	const { roomId } = req.params;
	let flag = 0;

	const room = await db.findField(collectionName, { $or: [{ roomId }, { roomId: parseInt(roomId) }] });
	if (!room) throw new Error('Invalid room Id');
	if (room.owner == uid) flag++;
	if (room.moderators && room.moderators.includes(uid)) flag++;

	if (flag < 1) throw new Error("Unauthorized! You do not have permissions for this operation");
	...
	let threadData = await Promise.all(threads.map(async (thread) => {
		let messageId = thread._key.split(':')[1];
		if (thread.reportedBy && Array.isArray(thread.reportedBy)) {
			let author = await User.getUserFields([thread.fromuid], ['picture', 'fullname', 'username', 'signature']);
			let threadreporters = await Promise.all(thread.reportedBy.map(async (item) => {
				let user = await User.getUserFields([item.uid], ['picture', 'fullname', 'username']);
				return { user, ...item }
			}));
			return { ...thread, reporters: threadreporters, messageId, author };
		} else return { ...thread, messageId, author };
	}));
	return utilities.paginate(`/app${req.url}`, threadData, count, limit, page);
}

appApi.highlightThread = async (req, payload) => {
	const collectionName = db.collections.DEFAULT;
	const uid = parseInt(req.uid);
	const { roomId, messageId } = req.body;
	const key = { _key: `message:${messageId}` };
	const room_key = { roomId: parseInt(roomId), type: 'discuss_room' };
	let flag = 0;

	let [message, room] = await Promise.all([
		db.findField(collectionName, key),
		db.findField(collectionName, room_key),
	]);
	if (!message) throw new Error("Invalid message Id");
	if (message.roomId != roomId || !room) throw new Error("Invalid room Id");

	if (room.owner == uid) flag++;
	if (room.moderators && room.moderators.includes(uid)) flag++;
	if (flag < 1) throw new Error("Unauthorized! You do not have permissions for this operation");

	const state = await db.updateField(collectionName, key, { $set: payload }, { upsert: false });
	return {
...
appApi.getHighlightedThreads = async (req) => {
	const collectionName = db.collections.DEFAULT;
	const { roomId } = req.params;
	const keys = {
		isHighlited: true,
	...
	let threadData = await Promise.all(threads.map(async (thread) => {
		let messageId = thread._key.split(':')[1];
		if (thread.fromuid) {
			let user = await User.getUserFields([thread.fromuid], ['picture', 'fullname', 'username', 'signature']);
			return { ...thread, author: user, messageId };
		} else return { ...thread, messageId };
	}));
	return utilities.paginate(`/app${req.url}`, threadData, count, limit, page);
}

appApi.saveThread = async (req) => {
	const collectionName = db.collections.DEFAULT;
	const uid = parseInt(req.uid);
	const { roomId, messageId } = req.body;
	const key = { _key: `message:${messageId}` };
	const _key = `discussion:room:${roomId}:saved`;

	let message = await db.findField(collectionName, key);
	if (!message) throw new Error("Invalid message Id");
	if (message.roomId != roomId) throw new Error("Invalid room Id");

	let saved = await db.findField(collectionName, { _key, uid });
	if (!saved) {
		await db.setField(collectionName, { _key, uid, 'messages': [messageId] });
	} else {
		await db.updateField(collectionName, { _key, uid }, { $addToSet: { 'messages': messageId } }, { upsert: false });
	}
	return { saved: true }
}

appApi.removeSavedThread = async (req) => {
	const collectionName = db.collections.DEFAULT;
	const uid = parseInt(req.uid);
	const { roomId, messageId } = req.body;
	const key = { _key: `message:${messageId}` };
	...
	let saved = await db.findField(collectionName, { _key, uid });
	if (saved) {
		await db.updateField(collectionName, { _key, uid }, { $pull: { 'messages': messageId } }, { upsert: false });
	}
	return { removed: true }
}

appApi.getSavedThreads = async (req) => {
	const collectionName = db.collections.DEFAULT;
	const uid = parseInt(req.uid);
	const { roomId } = req.params;
	const _key = `discussion:room:${roomId}:saved`;
	...
	let messages = await db.findField(collectionName, { _key, uid });
	if (messages && Array.isArray(messages.messages) && messages.messages.length) {
		...
		let start = page * limit;
		let end = start + limit;
		const Items = messages.splice(start, end);

		await Promise.all(Items.map(async (item) => {
			if (item) {
				let record = await db.findField(collectionName, { _key: `message:${item}` });
				messagesData.push(record);
			}
		}));

		let threadData = await Promise.all(messagesData.map(async (thread) => {
			let messageId = thread._key.split(':')[1];
			if (thread.fromuid) {
				let user = await User.getUserFields([thread.fromuid], ['picture', 'fullname', 'username', 'signature']);
				return { ...thread, author: user, messageId };
	...
	else return utilities.paginate(`/app${req.url}`, [], count, limit, page);
...
appApi.manageModerators = async (req, action) => {
	const uid = parseInt(req.uid);
	const roomId = parseInt(req.params.roomId);
	...
	if (typeof moderators == 'number') moderators = [moderators]

	if (!Array.isArray(moderators)) {
		throw new Error("Invalid moderators! Moderators should be an array of uids");
	}
	const keys = {
		type: 'discuss_room',
		roomId
	...
	const room = await db.findField(db.collections.DEFAULT, keys);
	if (!room) throw new Error("Room doesn't exists. Check RoomId");

	let roomModerators = room.moderators;
	if (room.owner != uid) throw new Error(`Unauthorized! Only the owner can manage moderators of this room`);

	if (action == 'add') {
		roomModerators = [...new Set([...roomModerators, ...moderators])];
	} else if (action == 'remove') {
		moderators.forEach((moderator) => {
			roomModerators.splice(roomModerators.indexOf(moderator), 1)
		});
	}

	await db.updateField(db.collections.DEFAULT, keys, { $set: { moderators: roomModerators, updatedAt: Date.now() } }, { upsert: false });
	return { updated: true };
}

appApi.updateRules = async (req) => {
	const uid = parseInt(req.uid);
	const roomId = parseInt(req.params.roomId);
	...
	const room = await db.findField(db.collections.DEFAULT, keys);
	if (!room) throw new Error("Room doesn't exists. Check RoomId");
	if (room.owner != uid) throw new Error(`Unauthorized! Only the owner can manage this room`);

	await db.updateField(db.collections.DEFAULT, keys, { $set: { rules: rules, updatedAt: Date.now() } }, { upsert: false });
	return { updated: true };
}
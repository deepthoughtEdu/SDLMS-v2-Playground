...
sdlmsAPI.getEagleBuilder = async function (data) {
	...
	if (parseInt(data.query.uid)) {
		let uid = parseInt(data.query.uid)
...
sdlmsAPI.updateSessionState = async function (data) {
	...
	let state = await db.updateFieldWithMultipleKeys(collectionName, {
...
sdlmsAPI.createSpreadSheet = async function (data) {
	...
	if (!topicId) {
		...
		let session = await db.findField(collectionName, { tid: topicId });
...
sdlmsAPI.getFeedbackData = async function (data) {
	...
	if (data.query.sessionIDs && data.query.uid) {
		...
		let sessionsFeedbackData = {}

		for (let i = 0; i < IDs.length && i < 10; i++) {
			let resp = await db.findFields(collectionName, {
				attachment_id: IDs[i],
				...query,
				type: "feedback"
			});
			sessionsFeedbackData[IDs[i]] = resp ? resp : [];
		}
	...
	if (data.query.uid) {
		...
		let feedbacks = await Promise.all(feedbackData.map(async (feedback) => {
			let userData = await User.getUserFields([feedback.asset_owner_uid], ['picture', 'fullname', 'username']);
			return {
				...feedback,
...
sdlmsAPI.getSessions = async function (data) {
	...
	if (query.child_uid) {
		let children = await groups.getChildren(uid);
	...
	if (type == 'previous') {
		sessions = await Promise.all(sessions.map(async (elem) => {
			let [threadbuilder, eaglebuilder] = await Promise.all([
				db.findField(collectionName, {
					topicId: elem.tid,
					userId: uid,
					type: 'threadbuilder'
...
sdlmsAPI.vote = async function (data) {
	...
	let feedback = await db.findField(collectionName, key);
	if (!feedback) {
	...
	let state = await db.updateFieldWithMultipleKeys(collectionName, key, {
		votes: votes,
...
sdlmsAPI.getBatches = async (data) => {
	...
	if (data.query.cid) {
		let cid = parseInt(data.query.cid);
		let batch = await db.findField(collectionName, {
			...key,
			cid: cid
		}, categoryFields)
		if (batch) {
...
async function getAssetSharableLink (data, type) {
	...
	let updated = await db.updateFieldWithMultipleKeys(collectionName, keys, {
		sharer: asset.sharer
	});
	if (!updated) throw new Error('Link has been Generated but Can\'t save it. Please Try again in a moment')
	return asset.sharer;
 ...
 */
 ...
 sdlmsAPI.comment = async function (req, res, next) {
	...
	let comment = await posts.create(data);
...
sdlmsAPI.editComment = async function (req, res, next) {
	...
	let comment = await posts.edit(data);
...
sdlmsAPI.getComments = async function (req, res, next) {
	...
	if (data.isNum) {
		if (data.toPid)
			comments = await db.findFields(collectionName, { typeOfAsset: 'comment', toPid: parseInt(data.toPid) });
		else
			comments = await db.findFields(collectionName, { typeOfAsset: 'comment', tid: parseInt(data.tid) });
	} else {
		if (data.toPid)
			comments = await db.findFields(collectionName, { typeOfAsset: 'comment', toPid: data.toPid });
		else
			comments = await db.findFields(collectionName, { typeOfAsset: 'comment', tid: data.tid });
	}
...
sdlmsAPI.createThought = async function (data) {
	const uid = parseInt(data.uid);
}
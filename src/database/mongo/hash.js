...
module.exports = function (module) {
	...
	module.update = async (collection, key, data) => {
		validateCollection(collection);
		let record = await module.client.collection(collection).update(key, data)
	...
	module.updateFieldWithMultipleKeys = async (collection, key, field) => {
		validateCollection(collection);
		if (!field) {return; }
		let record = await  module.client.collection(collection).findOne(key);
		if (!record) {
			return null;
		}
		else {
	...
	module.findField = async (collection, keys, fields=[]) => {
		validateCollection(collection);
		if (fields.length) {
			let record = await module.client.collection(collection).findOne(keys);
			if (record) {
				let filteredRecord = {};
				fields.forEach((field) => {
	...
	module.findLatestField = async (collection, keys) => {
		...
		let field = await module.client.collection(collection).find(keys).sort({$natural: -1}).limit(1).toArray()
		return field[0]
	}
	...
	module.getFieldsWithPagination = async (collection, keys, limit, page, order = { $natural: -1 }) => {
		validateCollection(collection);
		return await module.client.collection(collection).find(keys).sort(order).limit(limit).skip(page * limit).toArray()
	}
	...
	module.findFields = async (collection, keys, fields = []) => {
		validateCollection(collection);
		if (fields.length) {
			let records = await module.client.collection(collection).find(keys).toArray();

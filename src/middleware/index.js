 ...
 * @description Used in API calls to ensure that necessary file/data values are present
 * @param {Array} fields - Fields that needs to be checked for
 * @param {Object} req - Request body
 * @param {Object} res - Response
 * @param {Callback} next - Callback
 * @returns missing parameters for a particular API call
 */
middleware.checkRequiredFiles = function (fields, req, res, next) {
	var missingFields;
	if (req.files.files) {
		missingFields = fields.filter(field => !req.files.files.hasOwnProperty(field))
	} else {
	...
	controllers.helpers.formatApiResponse(400, res, new Error('Required file parameters were missing from this API call: ' + missingFields.join(', ')));
};
 ...
 * @description Checks if the user is logged in or not. If not logged in, it redirects back the user to the login page
 * @param {Object} req - Request body
 * @param {Object} res - Response
 * @param {Callback} next - Callback
 * @returns 
 */
middleware.requireLogin = function requireLogin (req, res, next) {
	if (!req.uid) {
